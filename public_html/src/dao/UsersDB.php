...
class UsersDB extends DatabaseAcess
{
    ...
    public function getPublicDataFromUserForID(): User
    {
        ...
        $query = $this->getConnection()->prepare('SELECT id, placing AS "index", name, image, CEP, state, city, rate, website, description, type, verified FROM users WHERE id = ?');
    ...
    {
        // Define query SQL para obter todas as colunas da linha do usuário
        $query = $this->getConnection()->prepare('SELECT id, placing AS "index", name, image, CEP, state, city, rate, website, description, type, verified FROM users WHERE placing = ?');
