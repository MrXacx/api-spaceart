...
foreach ($apiControllers as $route => $class) {
    ...
    Route::prefix("/$route")->name('user.alt.')->group(function () use ($route) {
        Route::post('/{id}/update', fn (Request $request) => redirect()->route("$route.update", $request->all()))->name('update');
        Route::post('/{id}/delete', fn (Request $request) => redirect()->route("$route.'.destroy", $request->all()))->name('destroy');
    });
}
